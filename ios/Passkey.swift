...
class Passkey: NSObject {
    ...
    @objc(
        register:withChallenge:withDisplayName:withUserId:withSecurityKey:withResolver:withRejecter:
    )
    func register(
        _ identifier: String, challenge: String, displayName: String, userId: String, securityKey: Bool,
        ...
        if #available(iOS 15.0, *) {
            ...
            if securityKey {
                ...
                if allowedCredentials.count > 0 {
                    authRequest.allowedCredentials = allowedCredentials.map {
